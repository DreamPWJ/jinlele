...
/**
 * @author pwj 2016/7/19 0019.
 *         日志统一处理切面类  记录日志,异常信息等方便调试  不侵入代码  解耦合
 *         日志打印在控制台和保存在jinlele.log文件中
 */
@Aspect
@Component
public class LogAdvice implements Ordered {
    ...
    @Pointcut("execution(* com.jinlele..*.*(..))")
    //所有的方法调用
    ...
    public Object interceptor(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (logger.isDebugEnabled()&&flag) {
            logger.info("===金乐乐珠宝程序运行方法点:  " + className + "." + methodName + "(" + argsvalue + ")" + " 方法进入开始！");
        }
        //proceed()就是调用目标方法 可控制目标方法是否执行
        Object object = pjp.proceed();
        //log4j.properties配置debug调试级别打印
        if (logger.isDebugEnabled()&&flag) {
            logger.info("===金乐乐珠宝程序运行方法点:  " + className + "." + methodName + "(" + argsvalue + ")" + " 方法返回结束！");
        }
        return object;
    }

    //异常记录  异常增强处理
    @AfterThrowing(pointcut = "execution(* com.jinlele..*.*(..))", throwing = "ex")
    public void interceptorException(Exception ex) throws Throwable {
        logger.info("=========  金乐乐珠宝程序异常错误记录  =========");
        logger.info("===金乐乐珠宝程序运行抛出的异常:    " + ex.getMessage());
        logger.info("===金乐乐珠宝程序异常发生的原因： " + ex.getCause());
        logger.info("===金乐乐珠宝程序运行异常详细信息：　　　" + ex.fillInStackTrace());
        logger.info("===金乐乐珠宝程序异常发生时间:    " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
    }
