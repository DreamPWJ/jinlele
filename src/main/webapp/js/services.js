...
//service在使用this指针，而factory直接返回一个对象
    ...
    .service('CancleOrderService',function($q, $http, JinLeLe){
        return {
            updateStatus:function(params){
                ...
                promise = $http({
                    method: 'GET',
                    url: JinLeLe.api + "/order/updateOrderStatus/" + params.orderno
                }).success(function (data) {
                    deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                }).error(function (err) {
                    deferred.reject(err);// 声明执行失败，即服务器返回错误
    ...
    .service('AddCommentService',function($q, $http, JinLeLe){
        return {
            getOrderDetail:function(params){
                ...
                promise = $http({
                    method: 'GET',
                    url: JinLeLe.api + "/order/getAddComment/" + params.orderno
                }).success(function (data) {
                    deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                }).error(function (err) {
                    deferred.reject(err);// 声明执行失败，即服务器返回错误
