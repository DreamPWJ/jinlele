...
//service在使用this指针，而factory直接返回一个对象
    ...
    .service('WalletService', function ($q, $http, JinLeLe) { //首页服务定义
        return{
             ...
             saveRechargeOrder:function(params) {
                 ...
                 promise = $http({
                     method: 'GET',
                     url: JinLeLe.api + "/order/saveRechargeOrder/" + params.userId +"/" + params.rechargeMoney
                 }).success(function (data) {
                     deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                 }).error(function (err) {
                     deferred.reject(err);// 声明执行失败，即服务器返回错误
    ...
    .service('GoodService', function ($q, $http, JinLeLe) { //商品相关的服务
        return {
            ...
            getExchangeGoodList: function (params) { //获取产品详情
                var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
                var promise = deferred.promise;
                promise = $http({
                    method: "GET",
                    url: JinLeLe.api + '/good/getExchangeGoodList/' + params.pagenow
                }).success(function (data) {
                    deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                }).error(function (err) {
                    deferred.reject(err);// 声明执行失败，即服务器返回错误
    ...
    .service('EvaluateService', function ($q, $http, JinLeLe) {
        return {
            ...
            getPMPrice:function(params){
                ...
                promise = $http({
                    method: 'GET',
                    url: JinLeLe.api + "/service/getPMPrice/"+ params.weight+'/'+ params.purity
                }).success(function (data) {
                    deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                }).error(function (err) {
                    deferred.reject(err);// 声明执行失败，即服务器返回错误
    ...
    .service('OrderService',function($q, $http, JinLeLe){
        return {
            ...
            addBarterInfo:function(params) {
                ...
                promise = $http({
                    method: 'GET',
                    url: JinLeLe.api + "/serviceOrder/addExchangeGood/" + params.orderno + "/" + params.goodId + "/" + params.goodchildId + "/" + params.buynum + "/" + params.unitprice + "/" + params.money
                }).success(function (data) {
                    deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
                }).error(function (err) {
                    deferred.reject(err);// 声明执行失败，即服务器返回错误
